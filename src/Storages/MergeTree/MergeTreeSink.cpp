...
void MergeTreeSink::finishDelayedChunk()
{
    ...
    for (auto & partition : delayed_chunk->partitions)
    {
        ...
        auto retry_times = 0;
        while (partition.block_with_partition.block->rows() > 0)
        {
            partition.temp_part->finalize();

            auto & part = partition.temp_part->part;
            auto block_ids = partition.block_with_partition.deduplication_info->getBlockIds(part->info.getPartitionId(), deduplicate);
            ...
            if (partition.block_with_partition.deduplication_info->isAsyncInsert())
...
std::vector<std::string> MergeTreeSink::commitPart(MergeTreeMutableDataPartPtr & part, const Block & , const std::vector<String> & block_ids)
{

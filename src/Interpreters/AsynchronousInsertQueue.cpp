    ...
    , data_kind(data_kind_)
{
    ...
    LOG_DEBUG(getLogger("AsynchronousInsertQueue"), "AsyncInsertQueue::InsertQuery data_kind: {}", data_kind);

    query->updateTreeHash(siphash, /*ignore_aliases=*/ true);

    auto tree_hash = query->getTreeHash(true);
    LOG_DEBUG(getLogger("AsynchronousInsertQueue"), "AsyncInsertQueue::InsertQuery tree hash: {}-{}", tree_hash.high64, tree_hash.low64);

    if (user_id)
    {
        siphash.update(*user_id);
        LOG_DEBUG(getLogger("AsynchronousInsertQueue"), "AsyncInsertQueue::InsertQuery user_id: {}", *user_id);
        for (const auto & current_role : current_roles)
        {
            siphash.update(current_role);
            LOG_DEBUG(getLogger("AsynchronousInsertQueue"), "AsyncInsertQueue::InsertQuery current role: {}", current_role);
        }
    }
    ...
    hash = siphash.get128();
    LOG_DEBUG(getLogger("AsynchronousInsertQueue"), "AsyncInsertQueue::InsertQuery hash computed: {}-{}", hash.items[0], hash.items[1]);
}
